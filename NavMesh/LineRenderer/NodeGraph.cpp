...
NodeGraph::NodeGraph(NavMesh* navMesh) : mNavMesh(navMesh)
{
	//ConstructNodeNeighbours(mNavMesh->GetTriangleList());
	//mNavMesh->AddNodeGraph(this);
}
...
void NodeGraph::ConstructNodeNeighbours(std::vector<Triangle>& triangleList)
{
	...
	//	edgeList[0] = { triangleList[i].mVerts[0], triangleList[i].mVerts[1] };
	//	edgeList[1] = { triangleList[i].mVerts[1], triangleList[i].mVerts[2] };
	//	edgeList[2] = { triangleList[i].mVerts[2], triangleList[i].mVerts[0] };
	//
	//	for (std::vector<Vec2> e : edgeList)
	//	{
	//		Triangle adjacentTriangle = FindAdjacentTriangleToEdge(i, e, mNavMesh->GetTriangleList());
	//		if (adjacentTriangle.mVerts.empty()) { continue; }
	//
